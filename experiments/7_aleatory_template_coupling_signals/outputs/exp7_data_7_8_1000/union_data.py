...
df1 = pd.read_csv('data_1000.csv').iloc[:731 * 9]  # Consideramos que cada sample tiene 9 registros
...
df2['i_sample'] += df1['i_sample'].max() + 1
df3['i_sample'] += df2['i_sample'].max() + 1
...
samples_to_keep = unique_samples[:-12]  # Mantener todas menos las últimas 12
...
print("Los últimos 12 samples han sido eliminados. Archivo guardado como 'data.csv'.")
