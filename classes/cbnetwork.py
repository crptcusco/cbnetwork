        ...
        for o_local_network in self.l_local_networks:
            ...
            for o_output_signal in l_output_edges:
                ...
                for signal_value in o_output_signal.d_out_value_to_attractor.keys():
                    for o_attractor in o_output_signal.d_out_value_to_attractor[signal_value]:
                        l_attractors_input.append(o_attractor)
                        o_attractor.show()
                # find the attractors that are generated by this signal
                l_attractors_output = self.get_attractors_by_input_signal(o_output_signal.index_variable)
                print("INFO:", "Input Attractor List ")
                for o_attractor in l_attractors_output:
                    o_attractor.show()
                print("-------------------------")

                # calculando los pares compatibles
                l_pairs_edge = list(itertools.product(l_attractors_input, l_attractors_output))
                # !!!!!!!!! ERROR
                o_output_signal.l_pairs_edge = l_pairs_edge
