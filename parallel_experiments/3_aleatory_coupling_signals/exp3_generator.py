...
N_SAMPLES = 10
N_LOCAL_NETWORKS = 8
GLOBAL_EDGES_LIST = list(range(7, 13))  # Se probará con 7, 8, 9, 10, 11 y 12 arestas globales
...
for i_sample in range(1, N_SAMPLES + 1):
    ...
    # Iterar sobre el número de arestas globales deseadas (7 a 12)
    for n_global_edges in GLOBAL_EDGES_LIST:
        # Para cada valor, crear una copia de la topología base y agregar arestas hasta alcanzar n_global_edges
        current_global_topology = copy.deepcopy(base_global_topology)
        while len(current_global_topology.l_edges) < n_global_edges:
            current_global_topology.add_edge()

        # Crear el template con el número fijo de variables
